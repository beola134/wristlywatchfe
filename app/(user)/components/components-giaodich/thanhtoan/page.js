...
export default function ThanhToan() {
  ...
  const fetchUserDetails = async (_id) => {
    try {
      const response = await fetch(`https://wristlywatchbe-bd4bdd62f0ed.herokuapp.com/users/${_id}`);
      if (!response.ok) {
  ...
  const applyDiscount = async () => {
    ...
    try {
      const response = await fetch(`https://wristlywatchbe-bd4bdd62f0ed.herokuapp.com/voucher/ma_voucher`, {
        method: "POST",
        headers: {
  ...
  const ktra = async () => {
    for (const items of cartItems) {
      const reponse = await fetch(`https://wristlywatchbe-bd4bdd62f0ed.herokuapp.com/product/check/${items._id}?quantity=${items.so_luong}`);
      if (!reponse.ok) {
        Swal.fire({
          title: "Không đủ hàng",
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (selectedPaymentMethod === "3") {
          try {
            const paymentResponse = await axios.post("https://wristlywatchbe-bd4bdd62f0ed.herokuapp.com/pttt/zalo", {
              amount: amount,
              orderDetails,
              headers: {
                "Content-Type": "application/json",
          ...
          try {
            const response = await fetch("https://wristlywatchbe-bd4bdd62f0ed.herokuapp.com/donhang/donhang", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
                ...
                <div className="flex w-full md:w-auto">
                  <div className={styles.productLeft}>
                    ...
                    <div className="w-20 h-auto">
                      <img src={`https://wristlywatchbe-bd4bdd62f0ed.herokuapp.com/images/${item.hinh_anh}`} alt={item.ten_san_pham} />
                    </div>
                  </div>
